...
TL;DR: 提出新的拓扑条件（不同方向的环态线性相关）来消除平带中的奇点，构造具有明确定义拓扑不变量的拓扑-拓扑（top²）平带，并研究其在相互作用下的行为。


<details>
  <summary>Details</summary>
Motivation: 传统拓扑平带（如Lieb或kagome模型）中的局域轨道满足的拓扑条件导致能带接触点处的Bloch态未定义，整个能带的拓扑不变量也无法定义。需要新的拓扑条件来消除奇点并获得明确定义的拓扑不变量。

Method: 提出新的拓扑条件：不同方向的环态（loop states）必须线性相关。满足此条件可消除能带接触点处的奇异性，并强制产生非平凡的、明确定义的拓扑不变量。基于此条件构造2D和3D的拓扑-拓扑（top²）平带。

Result: 获得了具有非平凡拓扑不变量的top²-平带，包括陈数、ℤ₂不变量和拓扑晶体不变量。在小的通用相互作用下，这些平带流向具有动态生成对称质量项的相关拓扑绝缘体；特殊设计的相互作用模型可以将top²-平带作为精确零模式。

Conclusion: 提出的新拓扑条件解决了传统拓扑平带中的奇点问题，使得整个能带具有明确定义的拓扑不变量。构造的top²-平带为研究相互作用下的拓扑物态提供了新平台，特别是在相关拓扑绝缘体和精确零模式模型方面。
...
TL;DR: 该论文在二维砖墙晶格中展示了混合皮肤-拓扑效应，结合了一阶能带拓扑与非互易跃迁，产生了不同于二阶皮肤效应的空间分布特征。


<details>
  <summary>Details</summary>
Motivation: 研究非厄米趋肤效应在更高维度的表现，探索拓扑与非厄米性之间的相互作用，特别是在二维系统中如何产生新的物理现象。

Method: 设计并研究二维砖墙几何结构中的非厄米晶格，结合一阶能带拓扑与非互易跃迁，分析开放边界条件下的谱特征，并通过拓扑电路实验进行验证。

Result: 发现混合皮肤-拓扑效应：角皮肤模式在复能平面上不显示非平凡绕数，而是表现出动态稳定的类例外点特征；只有源自厄米砖墙晶格拓扑角态的两个模式保持局域在单个角上，其余模式聚集在相对角对；空间分布与二阶皮肤效应明显不同。

Conclusion: 在二维砖墙晶格中成功实现了混合皮肤-拓扑效应，揭示了拓扑与非厄米性相互作用产生的新物理现象，并通过电路实验直接可视化了混合模式。
...
TL;DR: 该研究分析了石墨烯Corbino盘中的约瑟夫森效应，发现当内外半径比r2/r1≥5时，系统随电化学势和静电势垒空间分布变化，会从标准约瑟夫森隧穿(SJT)经石墨烯特有的多模狄拉克-约瑟夫森隧穿(MDJT)过渡到弹道约瑟夫森效应(BJE)。


<details>
  <summary>Details</summary>
Motivation: 先前Titov和Beenakker已理论描述了矩形几何超导-石墨烯-超导结中的约瑟夫森效应特征。本研究旨在将分析扩展到石墨烯Corbino盘结构，探索其独特的约瑟夫森效应行为。

Method: 采用狄拉克-博戈留波夫-德热纳方程分析石墨烯Corbino盘超导结，并辅以蜂窝晶格上量子输运的数值模拟进行验证。研究关注内外半径比r2/r1≥5的情况，分析电化学势和静电势垒空间分布变化对约瑟夫森效应的影响。

Result: 发现三种不同的约瑟夫森效应区域：SJT仅出现在狄拉克点附近且势垒形状接近矩形时；MDJT出现在三极区域且对势垒形状变化非常鲁棒；BJE在单极区域且势垒形状平滑时恢复。数值模拟结果与理论分析一致。

Conclusion: 石墨烯Corbino盘展示了丰富的约瑟夫森效应行为，包括标准隧穿、石墨烯特有的多模狄拉克隧穿和弹道效应之间的交叉转变，这些行为强烈依赖于电化学势和势垒形状，为石墨烯基约瑟夫森器件的设计提供了重要指导。
...
TL;DR: 研究混合正常-超导三量子点系统中的连续谱束缚态(BICs)，发现偏压、侧点失谐和超导邻近效应诱导弹性电子隧穿与Andreev反射通道间的干涉，导致精确的传输零点，标志着(准)BICs的出现。


<details>
  <summary>Details</summary>
Motivation: 探索混合正常-超导量子点系统中连续谱束缚态的形成机制和特性，特别是在存在电子-电子相互作用、有限偏压和超导邻近效应的情况下，理解弹性电子隧穿与Andreev反射通道之间的量子干涉如何导致传输零点和BICs的出现。

Method: 采用Hubbard近似处理局域电子-电子相互作用，研究由中央量子点（耦合到两个正常引线）和侧向量子点（邻近耦合到超导电极）组成的三量子点系统。通过分析有限偏压、侧点失谐和超导邻近效应诱导的弹性电子隧穿与Andreev反射通道之间的干涉，研究BIC相关模式和邻近诱导Andreev束缚态的作用。

Result: 当偏压扫过亚能隙共振时，弹性电子隧穿表现出尖锐的反共振，并演变为精确的传输零点，标志着(准)BICs的出现。随着失谐不对称性增加，观察到从Fano-Andreev BIC支持机制到Fano-Andreev准BIC机制的连续交叉。BICs和准BICs的形成伴随着侧量子点占据数的显著变化，提供了与束缚态传输特征直接相关的内部诊断。

Conclusion: 混合正常-超导三量子点系统中，通过偏压、失谐和超导邻近效应的协同作用，可以实现连续谱束缚态和准束缚态的形成。这些态表现为传输零点，并通过侧量子点占据数的变化提供内部诊断，为量子干涉和超导-正常混合系统中的束缚态研究提供了新见解。
...
### [5] [Magnetic field Controlled Anderson Delocalization in a Spinful Non-Hermitian Chain](https://arxiv.org/abs/2603.25700)
*Moirangthem Sanahal,Subhasis Panda,Snehasish Nandy*

Main category: cond-mat.mes-hall

TL;DR: 在自旋非厄米系统中，磁场可作为额外自由度调控局域化行为，通过Zeeman效应诱导的自旋通道间耦合有效抑制无序强度，实现Anderson局域化向非厄米趋肤效应的转变，甚至在强无序系统中促进Anderson退局域化转变。


<details>
  <summary>Details</summary>
Motivation: 研究自旋非厄米系统中无序、非厄米性和磁场三者的相互作用，探索磁场如何调控Anderson局域化(AL)与非厄米趋肤效应(NHSE)之间的竞争关系，揭示磁场诱导的退局域化新机制。

Method: 研究一维自旋非厄米无序链，分析在自旋通道间适当关联无序构型下，磁场对局域化行为的影响。通过计算逆参与率和平均质心位置，绘制无序、非厄米性和磁场强度三者相互作用的相图，识别Anderson局域化和趋肤积累区域。

Result: 磁场增强AL→NHSE转变，在强无序系统中促进Anderson退局域化转变。磁场通过Zeeman效应诱导的自旋通道间耦合有效抑制无序强度，实现局域化行为的调控。相图揭示了Anderson局域化和趋肤积累的不同区域。

Conclusion: 自旋非厄米系统中磁场作为关键调控参数，通过Zeeman耦合机制实现无序强度的有效抑制，为Anderson局域化向非厄米趋肤效应的转变提供了新途径，揭示了磁场诱导退局域化的物理机制。
...
TL;DR: 在AB堆叠双层石墨烯中，考虑三角扭曲效应后，较小的面内磁场（~10 T）和横向电场（阈值~0.6 meV）即可实现绝缘体-金属转变，相比之前预测大幅降低了所需磁场强度。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明AB堆叠双层石墨烯在面内磁场和横向电场作用下会发生绝缘体-金属转变，但所需磁场强度过大而不实用。本文旨在通过考虑之前被忽略的三角扭曲效应（通过层间倾斜耦合实现），探索是否能在更小的磁场下实现类似的转变。

Method: 在动量空间受限区域（约为原始布里渊区的1/100）引入三角扭曲效应，该效应通过层间倾斜耦合实现，导致狄拉克锥的精细分裂。分析在无外场时形成补偿半金属态，然后研究施加横向电场（阈值~0.6 meV）和面内磁场对能隙的影响。

Result: 三角扭曲效应在无外场时产生补偿半金属态。施加超过阈值（~0.6 meV）的横向电场会恢复绝缘能隙，但该能隙可被相对较小的面内磁场（约10 T）关闭，从而实现绝缘体-金属转变，所需磁场远小于先前预测。

Conclusion: 考虑三角扭曲效应后，AB堆叠双层石墨烯中实现绝缘体-金属转变所需的磁场强度大幅降低至约10 T，这为实验实现提供了更可行的条件，并揭示了层间倾斜耦合在调控电子态中的关键作用。
...
TL;DR: 该论文研究了双量子点Kitaev链中电电流的涨落，通过微分有效电荷q（微分散粒噪声与电导之比）来表征。发现q在低偏压下的不同取值可以区分Majorana态区域，高偏压下q=3e/2能唯一识别poor man's Majorana态，揭示了最小Kitaev链中丰富的涨落行为。


<details>
  <summary>Details</summary>
Motivation: 最小Kitaev链为工程化Majorana态提供了独特平台，但对其非平衡涨落行为的理解尚不充分。需要研究电电流涨落特性来表征和识别Majorana态，特别是通过微分有效电荷这一物理量来揭示最小Kitaev链中独特的涨落行为。

Method: 采用双量子点Kitaev链模型，分析电电流的涨落特性。通过计算微分有效电荷q（微分散粒噪声与电导之比），研究在不同偏压V和参数条件下（正常隧穿振幅|η_n|与交叉Andreev反射振幅|η_a|）的涨落行为。特别关注低偏压和高偏压极限下的q值变化。

Result: 1. 低偏压下：在|η_n|=|η_a|的极窄区域内q=e/2；在几乎整个sweet spot区域q=3e/2，表明poor man's Majorana态主导涨落。2. 高偏压下：sweet spot区域仍保持q=3e/2，可唯一识别poor man's Majorana态。3. 当|η_n|=0或|η_a|=0时，任何偏压下q=e。4. 在达到渐近值q=e之前，|eV|=2√(|η_n|²+|η_a|²)处出现最大值q=2e。

Conclusion: 最小Kitaev链展现出独特且丰富的电涨落行为，微分有效电荷q可作为识别Majorana态的有效探针。高偏压下的q=3e/2特征能唯一识别poor man's Majorana态，为下一代非平衡散粒噪声实验提供了理论基础和实验指导。
...
TL;DR: 提出了反常Floquet拓扑相在实空间中的局域探测方法：通过测量局域粒子在Floquet周期内包围的面积来指示拓扑不变量，当面积接近半晶胞时标志反常相存在。


<details>
  <summary>Details</summary>
Motivation: 反常Floquet拓扑相打破了基于陈数的体边对应关系，其拓扑由绕数描述。目前缺乏类似陈标记的局域体指标来探测Floquet系统的绕数，这限制了在无序或相互作用系统中的实际探测。

Method: 研究二能带模型，分析局域粒子在Floquet周期内包围的面积。在微运动无色散动力学的精细调节点，证明包围面积是量子化的，并与绕数存在精确比例关系。

Result: 发现当包围面积接近半晶胞面积时，系统处于反常拓扑相。在无色散动力学条件下，面积量子化且与绕数成正比。基于此关系，展示了实现任意高绕数的方法。

Conclusion: 提出了反常Floquet拓扑相在实空间中的局域探测方案，为量子模拟平台中直接探测拓扑相提供了新途径，特别适用于无序或相互作用系统。
...
TL;DR: 本文提出轨道有序作为二维金属反铁磁体的通用微观机制，并以单层YbMn₂Ge₂为例展示了巨大的非相对论性自旋劈裂和可调控的横向自旋电导。


<details>
  <summary>Details</summary>
Motivation: 二维金属反铁磁体材料稀少，且尚未发现具有大非相对论性自旋劈裂的关联二维材料。需要探索新的微观机制来实现这种材料体系。

Method: 提出自发轨道有序作为实现二维金属反铁磁体的通用机制：通过电子关联和费米面嵌套驱动的反铁磁轨道有序（dₓz和dᵧz轨道之间），打破具有相反自旋的磁性子晶格的等价性，产生对称性强制反铁磁自旋纹理。以单层YbMn₂Ge₂作为具体实现案例进行研究。

Result: 单层YbMn₂Ge₂被确定为稳定的关联金属反铁磁体，展现出约1 eV量级的巨大非相对论性自旋劈裂。该相支持异常大且可通过门电压调控的横向自旋电导。

Conclusion: 关联驱动的轨道有序为设计具有大自旋劈裂的关联反铁磁体提供了一种鲁棒且通用的机制，为二维自旋电子学材料开辟了新途径。
...
TL;DR: 该论文系统研究了蜂窝状纳米带中的端态，重点分析了纳米带终止方式、手性对称性和复杂次近邻跃迁之间的相互作用，使用多带Zak相表征占据带的拓扑性质，揭示了端态存在和鲁棒性的终止依赖性。


<details>
  <summary>Details</summary>
Motivation: 先前研究虽然识别了蜂窝状纳米带中的零维端态，但分析不完整。需要系统研究不同宽度和终止方式的纳米带，阐明端态存在和鲁棒性的终止依赖性，以及复杂次近邻跃迁相位变化对端态的影响。

Method: 采用Haldane模型框架，系统研究不同宽度和终止方式（锯齿型和扶手椅型）的纳米带。使用多带Zak相表征占据带的拓扑性质，分析Zak相的量子化条件。研究复杂次近邻跃迁相位变化对系统的影响，包括手性对称性破缺、体能隙演化和端态能量去钉扎。

Result: 发现Zak相仅在特定纳米带终止方式下量子化，这种终止依赖性决定了端态的存在和鲁棒性。复杂次近邻跃迁相位变化导致手性对称性破缺、体能隙演化，进而引起端态能量去钉扎。研究结果与Kane-Mele模型建立了联系，包括Rashba自旋轨道耦合的作用。

Conclusion: 该工作提供了Haldane和Kane-Mele模型中纳米带拓扑端态的详细分析，建立了一个表征相关系统中端态的框架，阐明了终止依赖性和复杂跃迁参数对端态性质的关键影响。
...
### [11] [Anomalous thermoelectric Hall response of interacting 2D Dirac fermions](https://arxiv.org/abs/2603.25705)
*A. Daria Dumitriu-I.,Feng Liu,Alexander E. Kazantsev,Alessandro Principi*

Main category: cond-mat.mes-hall

TL;DR: 二维有质量狄拉克费米子的反常热电霍尔响应在电子-电子相互作用的一阶近似下被研究，发现减去磁化电流后热电霍尔系数在零温极限下并不消失，这归因于量子场论中不可避免的小尺度非局域性。


<details>
  <summary>Details</summary>
Motivation: 研究二维有质量狄拉克费米子在电子-电子相互作用下的反常热电霍尔响应，特别关注相互作用对热输运性质的影响以及传统理论预期的验证。

Method: 计算了Luttinger型引力势下的能斯特响应和粒子磁化，后者用于消除虚假的非输运贡献。在电子-电子相互作用的一阶近似下，推导了磁化的简洁公式，并分析了减去磁化电流后的热电霍尔系数行为。

Result: 发现对于任意相互作用，磁化可以用一个极其简单的公式描述。令人惊讶的是，减去磁化电流后，热电霍尔系数在零温极限下并不消失，这与预期相反。这归因于量子场论中不可避免的最小长度尺度上的非局域性，这种非局域性在红外物理中显现出来。

Conclusion: 二维有质量狄拉克费米子的反常热电霍尔响应揭示了量子场论中固有的小尺度非局域性对宏观输运性质的影响，这种非局域性即使在红外极限下也会影响热电霍尔系数，挑战了传统理论预期。
...
### [12] [Origin of Giant Phonon Magnetic Moment in Orbital Seebeck Effect: a Heisenberg-type L-L Coupling](https://arxiv.org/abs/2603.25019)
*Hong Sun,Jinxin Zhong,Yimin Yao,Jun Zhou,Lifa Zhang*

Main category: cond-mat.other

TL;DR: 在α-石英中观测到轨道塞贝克效应的启发下，本文发现手性绝缘体中存在热产生声子角动量和声子磁矩的本征放大机制，提出声子角动量间的海森堡型长程耦合（L-L耦合），通过自洽反馈通道将热响应放大近两个数量级。


<details>
  <summary>Details</summary>
Motivation: 受α-石英中观测到的轨道塞贝克效应启发，探索手性绝缘体中热产生声子角动量和声子磁矩的放大机制，为理解巨大声子磁矩的微观起源提供理论框架。

Method: 提出声子角动量间的海森堡型长程耦合（L-L耦合），建立自洽反馈通道，在线性响应理论框架下计算热响应的放大效应，分析对角和非对角通道的贡献。

Result: 计算显示显著的温度和尺寸依赖放大效应，以非对角通道为主导，当系统从下方接近阈值时，总声子角动量可增强近两个数量级。

Conclusion: L-L耦合可能为α-石英中观测到的巨大声子磁矩和轨道塞贝克效应提供微观起源，揭示了手性绝缘体中热产生声子角动量的本征放大机制。
...
### [13] [Implementation of the multigrid Gaussian-Plane-Wave algorithm with GPU acceleration in PySCF](https://arxiv.org/abs/2603.24881)
...
<details>
  <summary>Details</summary>
Motivation: 传统Kohn-Sham密度泛函理论中的Fock矩阵构建和核梯度计算计算成本高，需要高效的GPU加速方法以支持大规模分子和固体体系的计算，如从头算分子动力学和高通量计算。

Method: 开发了GPU加速的多重网格高斯-平面波密度拟合方法，采用基于网格的并行化策略收缩高斯基函数对，在CUDA内核中实现，支持高角动量函数（最高f壳层），无效率损失。

Result: 在NVIDIA GPU上达到FP64峰值性能的80%，对含1536个原子和20480个基函数的分子和固体体系，在H100 GPU上相比28核共享内存节点CPU实现获得最高25倍加速。256个水分子团簇的基态能量和核梯度计算可在单H100 GPU上约30秒内完成。
...
### [14] [Enabling ab initio geometry optimization of strongly correlated systems with transferable deep quantum Monte Carlo](https://arxiv.org/abs/2603.25381)
*P. Bernát Szabó,Zeno Schätzle,Frank Noé*

Main category: physics.chem-ph

TL;DR: 结合可迁移深度学习变分蒙特卡洛与高斯过程回归，实现高精度从头算势能面探索，包括结构弛豫、过渡态搜索和最小能量路径计算。


<details>
  <summary>Details</summary>
Motivation: 化学过程需要探索分子势能面的广阔区域，但对于强关联体系仍具挑战性。可迁移深度学习VMC能高效求解电子薛定谔方程，但其随机性使得直接探索分子构型空间变得困难。

Method: 结合可迁移深度学习VMC与高斯过程回归：在VMC优化电子波函数时连续采样核构型，获得可迁移描述；稀疏评估势能面，通过采样几何构型估计VMC能量和力，使用高斯过程回归聚合噪声数据构建局部近似。

Result: 方法在化学相关分子几何分布内实现零样本化学精度，能够准确高效探索复杂势能面景观，包括结构弛豫、过渡态搜索和最小能量路径计算，适用于基态和激发态。

Conclusion: 该方法为研究具有显著多参考特征的体系中的键断裂、形成和大结构重排打开了大门，实现了强关联体系势能面的高精度从头算探索。
...
### [15] [Adaptive finite volume-particle method for free surface flows](https://arxiv.org/abs/2603.25130)
...
<details>
  <summary>Details</summary>
Motivation: 传统自由表面流动模拟方法存在精度与效率的权衡问题：纯SPH方法计算成本高，而纯网格方法在界面追踪方面存在困难。需要一种能够结合两者优势的混合方法。

Method: AFVPM将非结构网格上的有限体积法用于主体流动区域以利用其计算效率和数值精度，同时在界面附近采用弱可压缩SPH公式以保持鲁棒的自由表面追踪能力。关键创新包括：基于块的动态自适应欧拉网格-拉格朗日粒子转换策略、基于缓冲区的单元-粒子算法确保数据无缝通信，以及在网格区域使用包含重力效应的等温气体动力学方案计算通量。

Result: 通过一系列涉及复杂自由表面现象的基准案例验证了AFVPM的性能和可靠性。数值结果表明，与纯SPH方法相比，AFVPM实现了更高的精度和效率。

Conclusion: AFVPM成功地将欧拉网格方法和拉格朗日粒子方法的优势结合起来，为自由表面流动模拟提供了一种既准确又高效的混合框架，在保持界面追踪鲁棒性的同时显著提高了计算效率。
...
TL;DR: 利用皮秒电脉冲在超导材料中实现接近本征临界电流密度Jc*的测量，突破了传统直流测量的涡旋运动限制，为研究超导微观特性和发展超导电子学提供了新方法。


<details>
  <summary>Details</summary>
Motivation: 传统直流测量中，II类超导体的实验临界电流密度Jc通常低于本征极限Jc*，主要受涡旋运动和自热效应限制。需要开发新方法来直接探测本征临界电流密度，这对理解超导微观特性和提高高磁场器件及电力系统的电流极限具有重要意义。

Method: 采用超快皮秒电脉冲技术，在涡旋惯性固定的时间尺度内与材料相互作用，测量超电流直至本征退配对极限Jc*。以NbN（s波超导体）和YBa2Cu3O7（d波超导体）为代表材料进行皮秒临界电流探测。

Result: 在NbN中观察到皮秒退配对的急剧起始，临界电流密度高达Jc*=2.2 Jc，与基于BCS理论的微观动力学描述相符。而YBCO则表现出随皮秒电流增加的超导性逐渐抑制，反映了其d波对称性特征。

Conclusion: 皮秒电脉冲技术提供了一种超越传统输运测量能力的新型超导体探测手段，能够达到退配对电流极限，为发展稳健的超导电子学平台开辟了新途径。
...
TL;DR: 该研究通过隧道谱和超流刚度测量，揭示了扭曲石墨烯中有限动量对密度波（PDW）态的存在，该态能同时解释低能隧道权重和低温超流刚度的T²抑制现象。


<details>
  <summary>Details</summary>
Motivation: 扭曲石墨烯超导性中存在一个关键矛盾：隧道谱显示显著的零偏压电导（表明存在无能隙准粒子），而超流刚度在低温下却呈现T²抑制（通常与无能隙激发不相容）。需要一种理论框架来统一解释这些看似矛盾的实验观测。

Method: 提出有限动量对密度波（PDW）态作为理论模型。该模型产生Bogoliubov费米面和有限零偏压电导。通过分析PDW序对几何超流响应的影响，推导出低温刚度的T²抑制行为，即使在平带极限下也成立。

Result: PDW态能同时解释：1）显著的零偏压隧道电导；2）低温超流刚度的T²抑制；3）预测在密度或位移场调控下，增强的残余零偏压电导应伴随降低的低温刚度，为实验提供直接验证线索。

Conclusion: 有限动量PDW态为扭曲石墨烯超导性提供了自洽的理论框架，统一了隧道谱和超流刚度的矛盾观测。该模型预测了可实验验证的关联特征，为理解这类非常规超导体的配对机制提供了新视角。
...
TL;DR: 该论文提出了一个可精确求解的模型，用于研究欠掺杂铜氧化物超导体中费米弧与d波超导性的竞争关系，解析地展示了费米弧如何抑制超导转变温度Tc并导致gap-to-Tc比值超过平均场极限。


<details>
  <summary>Details</summary>
Motivation: 欠掺杂铜氧化物超导体的正常态是神秘的费米弧金属态，需要建立可精确求解的模型来理解费米弧与d波超导性之间的竞争关系，为高温超导机制提供解析基准。

Method: 采用最近提出的可精确求解模型，该模型能清晰地展示费米弧的出现。通过引入d波配对相互作用，获得超导转变温度Tc的渐近精确解，并解析地分析费米弧产生的多体效应。

Result: 模型产生了随空穴掺杂变化的超导穹顶状Tc曲线，与铜氧化物实验观测定性一致。解析证明费米弧产生额外的多体效应，不仅通过费米面收缩抑制Tc，还导致gap-to-Tc比值显著超过平均场极限。

Conclusion: 这些发现为理解高温超导体中费米弧物理如何与d波超导性竞争提供了解析基准，揭示了费米弧的多体本质对超导特性的重要影响。
...
<details>
  <summary>Details</summary>
Motivation: 竞争性电荷序和自旋序是理解非常规超导性质的关键。La₃Ni₂O₇在高压下表现出高温超导性，但其在常压下的电荷序结构尚不明确，需要揭示其晶体结构对称性破缺

Method: 利用同步辐射X射线衍射技术，对高质量单晶样品进行研究。通过高光子通量和大的动态范围，探测到强度比主布拉格反射弱近四个数量级的微弱反射

Result: 观测到之前衍射研究中被忽略的微弱反射，证明晶体结构打破了滑移镜面对称性，形成了极性结构而非广泛使用的中心对称结构模型。极性由镍位点上的棋盘状电荷序和八面体倾斜共同诱导产生

Conclusion: 该研究为理解双层镍酸盐中的相竞争和压力诱导超导机制提供了基础，揭示了电荷序与超导相之间的竞争关系
...
TL;DR: 通过时变Ginzburg-Landau理论模拟多层异质结构，揭示了超导二极管效应的微观机制源于洛伦兹力与不对称涡旋动力学的相互作用，且可通过改变层堆叠顺序完全抑制该效应。


<details>
  <summary>Details</summary>
Motivation: 超导二极管效应（SDE）在量子技术和节能器件中具有重要应用潜力，但其微观机制尚不明确。本文旨在通过模拟多层异质结构来阐明SDE的微观机制，特别是涡旋动力学的作用。

Method: 使用时变Ginzburg-Landau理论模拟非对称多层异质结构。系统改变由铌、钒、钽构成的三层结构中的层厚度、外磁场和堆叠顺序，这些参数与开创性实验工作相似，以阐明涡旋动力学的作用。

Result: 模拟显示显著的SDE源于洛伦兹力与不对称涡旋动力学的相互作用，且强烈依赖于层堆叠顺序。通过简单改变组成层的堆叠顺序，可以完全抑制SDE。

Conclusion: 这些发现为SDE的微观机制提供了深入见解，并为在实际超导器件中控制和消除SDE提供了可行方法。
...
TL;DR: 非微扰蒙特卡洛研究显示，二维d波交变磁超导体中的有限动量超导性可在无外磁场条件下稳定存在，交变磁性使配对密度波态具有热稳定性。


<details>
  <summary>Details</summary>
Motivation: 研究交变磁性如何促进有限动量超导性，特别是配对密度波态在无外磁场条件下的稳定性和热稳定性，探索实现热稳定PDW超导的新途径。

Method: 采用超越平均场理论的非微扰蒙特卡洛方法，研究二维d波交变磁超导体中的有限动量超导性。

Result: 交变磁性可在无外磁场条件下稳定配对密度波态，并使其在有限温度下保持鲁棒的相位相干性，为热稳定PDW超导提供了可行路径。

Conclusion: 交变磁性是实现热稳定配对密度波超导性的有前景途径，研究识别了明确的热力学和光谱学特征信号。
...
TL;DR: 提出了一种基于多体机制的强约瑟夫森二极管效应，通过双量子点SQUID中的分支选择机制实现高效非互易超导电流。


<details>
  <summary>Details</summary>
Motivation: 传统约瑟夫森二极管效应通常依赖于单个基态内电流-相位关系的非对称性，效率有限。本文旨在探索通过多体物理机制增强二极管效应的方法，特别是在纳米尺度SQUID系统中。

Method: 构建由两个平行量子点耦合到超导引线组成的相互作用纳米SQUID系统。采用有效模型在超导原子极限下分析，并通过广义原子极限框架验证有限能隙器件的鲁棒性。重点研究非局域库珀对隧穿通道的作用。

Result: 发现了一种分支选择的约瑟夫森二极管效应：正负临界电流在跨越0-π相边界的不同多体分支上分别优化，显著增强了二极管效率。非局域库珀对隧穿通道重塑了0-π边界，在参数空间中产生了明显的"二极管带"，与仅存在局域库珀对转移时的脆弱热点形成鲜明对比。

Conclusion: 多体相互作用和非局域库珀对隧穿是实现强约瑟夫森二极管效应的关键机制。该分支选择机制在超导原子极限下有效，且结论对实际有限能隙器件具有鲁棒性，为设计高效超导二极管器件提供了新途径。
...
TL;DR: 量子限制与邻近效应协同作用可显著增强双层异质结构的超导临界温度，甚至在单组分非超导或弱超导材料中诱导出超导性。


<details>
  <summary>Details</summary>
Motivation: 纳米尺度超导性的核心挑战在于理解和利用量子限制与邻近效应在实际金属异质结构中的协同作用。这两种效应在实验现实的异质结构中如何共存并相互作用尚未得到充分探索。

Method: 采用广义Eliashberg理论框架，该框架同时包含了量子限制效应和邻近耦合效应。通过这一理论模型研究双层异质结构中两种效应的相互作用机制。

Result: 理论预测表明，量子限制与邻近效应的协同作用可以显著提升超导临界温度。特别值得注意的是，在某些双层结构中，即使组成材料在体相状态下是非超导或仅具有弱超导性，也能通过这种协同效应诱导出超导性。

Conclusion: 量子限制双层结构是工程化金属异质结构中涌现超导性的有前景途径。这种协同效应为设计新型超导材料提供了理论指导。
...
### [24] [Bound states of anyons: a geometric quantization approach](https://arxiv.org/abs/2603.24701)
*Qingchen Li,Pavel A. Nosov,Taige Wang,Eslam Khalaf*

Main category: cond-mat.str-el

TL;DR: 提出一种在任意子希尔伯特空间内研究任意子束缚的可控可扩展方法，应用于ν=1/3 Laughlin态准空穴，发现纯排斥相互作用下仍能形成束缚态，这是Berry相位效应驱动的。


<details>
  <summary>Details</summary>
Motivation: 任意子相互作用及其可能的束缚问题在分数量子霍尔态物理中至关重要，但传统方法难以直接研究任意子束缚现象。

Method: 在任意子希尔伯特空间内工作，引入有效势（捕获经典任意子构型的静电能量）和Kähler势（同时编码任意子Berry相位及其希尔伯特空间结构），通过蒙特卡洛方法计算，应用Kähler流形上的几何量子化构造任意子哈密顿量，在少任意子希尔伯特空间中进行精确对角化。

Result: 对于ν=1/3 Laughlin态准空穴，当屏蔽长度小于或等于磁长度时，Laughlin准空穴形成束缚态；尽管电子-电子相互作用和准空穴静电势都是纯排斥的，束缚仍会发生，这是Berry相位效应驱动的，由准空穴密度分布在ℓ_B尺度上的振荡引起；随着屏蔽长度减小，观察到相序列：自由e/3任意子→配对2e/3束缚态→三任意子电荷-e团簇→更大的复合物。

Conclusion: 该方法为研究任意子束缚提供了可控框架，揭示了Berry相位在任意子束缚中的关键作用，对量子霍尔系统的电荷成像实验和分数量子反常霍尔材料中巡游任意子相的相图具有重要启示。
...
### [25] [Topological properties of gapless phases in an interacting spinful wire](https://arxiv.org/abs/2603.24831)
*Polina Matveeva,Dmitri Gutman,Sam T. Carr*

Main category: cond-mat.str-el

TL;DR: 该论文研究了具有自旋-电荷分离的相互作用自旋模型中无能隙相位的拓扑性质，发现了两种拓扑非平庸的无能隙态：拓扑Luther-Emery液体和拓扑Mott绝缘体，它们分别具有分数化自旋和电荷的边缘态。
...
Method: 通过分析具有自旋-电荷分离的相互作用自旋模型，研究Z2对称性破缺相之间的无能隙边界。使用拓扑Luther-Emery液体和拓扑Mott绝缘体的理论框架，并与非相互作用的拓扑金属建立绝热连接。

Result: 发现了两种拓扑非平庸的无能隙态：1) 拓扑Luther-Emery液体：具有无能隙电荷激发和携带分数化自旋的拓扑边缘模；2) 拓扑Mott绝缘体：具有无能隙自旋激发和携带分数化电荷的边缘态。这些相互作用态可以绝热连接到非相互作用的拓扑金属。

Conclusion: 相互作用系统中的无能隙相可以表现出丰富的拓扑性质，即使没有局域序参量或平均场描述。拓扑Luther-Emery液体和拓扑Mott绝缘体代表了新的拓扑相，它们通过分数化边缘态和与Su-Schrieffer-Heeger链的拓扑金属的绝热连接来表征。
...
### [26] [Dynamical Response of the Kitaev Spin Liquid under Third-Nearest-Neighbor Heisenberg Interaction](https://arxiv.org/abs/2603.24918)
*Chuan Chen,Jiucai Wang*

Main category: cond-mat.str-el

TL;DR: 研究Kitaev自旋液体在第三近邻海森堡相互作用J3扰动下的动力学性质，发现J3诱导出相干准磁振子模式，与Majorana连续谱共存，并导致量子相变到磁有序态。


<details>
  <summary>Details</summary>
Motivation: 实验证据表明第三近邻海森堡相互作用J3在候选Kitaev材料中具有重要性，需要研究其对Kitaev自旋液体动力学性质的影响。

Method: 采用自洽部分子平均场加随机相位近似框架，计算自旋动力学结构因子和拉曼散射响应，分析J3扰动下的集体模式和相变。

Result: J3诱导出相干准磁振子模式，与高能Majorana连续谱共存；模式软化标志量子相变到磁有序态；铁磁和反铁磁Kitaev模型在相同临界J3处发生磁有序；外磁场进一步软化模式增强磁有序；拉曼响应显示不同顶点探测Majorana费米子和visons激发。

Conclusion: 研究为J3扰动的Kitaev自旋液体动力学特征提供了统一视角，有助于解释具有显著J3相互作用的候选Kitaev材料的实验谱。
...
### [27] [Microscopic nature of $4a_0\times4a_0$ plaquettes in stripe LDOS and $2a_0$ shift](https://arxiv.org/abs/2603.25245)
*Ying Liang,Yi-Da Chu,Shi-Jie Hu,Xue-Feng Zhang*

Main category: cond-mat.str-el

TL;DR: 该研究使用量子色弦模型框架揭示了铜酸盐中普遍存在的4a₀×4a₀斑块微观起源与spinon单重态对相关，并识别出粒子-空穴对称性破缺导致的2a₀偏移效应。


<details>
  <summary>Details</summary>
Motivation: 扫描隧道显微镜（STM）是可视化铜酸盐中与超导性紧密交织的局域态密度（LDOS）的有力工具。奇特的LDOS图案模式是揭示潜在微观机制的关键，但需要新的波函数基础视角来解释实验中的STM信号。

Method: 采用量子色弦模型框架，分析铜酸盐中条纹的局域态密度。通过比较模拟数据与铜酸盐LDOS实验数据，识别粒子-空穴对称性破缺效应，并在更长的条纹（L=18）中进行验证。

Result: 揭示了4a₀×4a₀斑块的微观起源与spinon单重态对密切相关。识别出粒子-空穴对称性破缺导致的2a₀偏移效应，该效应在更长的条纹中得到确认。

Conclusion: 该工作为解释实验中STM信号提供了新的波函数基础视角，可能推进对高温超导铜酸盐微观机制的理解。
...
### [28] [Structural and magnetic phases of topological kagome metal Fe$_3$Sn$_2$ under pressure](https://arxiv.org/abs/2603.25271)
*S. Chattopadhyay,L. Thomarat,C. S. Ong,K. Kargeti,Lipika,J. -P. Rueff,L. Nataf,K. Manna,S. K. Panda,C. Shekhar,V. Balédent*

Main category: cond-mat.str-el

TL;DR: Fe₃Sn₂在高压下经历结构相变和自旋态转变，压力通过增强自旋-轨道耦合和DM相互作用重塑其磁性能量景观，为调控kagome材料的磁各向异性和拓扑相提供了有效手段。
...
Method: 结合X射线衍射(XRD)、X射线发射光谱(XES)、X射线磁圆二色性(XMCD)和自旋极化密度泛函理论(DFT)计算，系统研究高压下Fe₃Sn₂的结构和磁性演化。

Result: 在~20 GPa以上观察到结构相变，伴随Fe局域磁矩的显著减小（高自旋到低自旋转变）；建立了丰富的压力-温度磁相图，发现压力增强自旋-轨道驱动的磁各向异性和Dzyaloshinskii-Moriya相互作用，有利于非共线磁构型。
...
### [29] [Flat band driven competing charge and spin instabilities in the altermagnet CrSb](https://arxiv.org/abs/2603.25317)
*A. Korshunov,M. Alkorta,C. -Y. Lim,F. Ballester,Cong Li,Zhilin Li,D. Chernyshov,A. Bosak,M. G. Vergniory,Ion Errea,S. Blanco-Canosa*

Main category: cond-mat.str-el

TL;DR: 在CrSb中观察到短程电荷序涨落与自旋序竞争，在TN处出现显著的Kohn异常和巨大的自旋-声子耦合(~6 meV)，由平带物理放大


<details>
  <summary>Details</summary>
Motivation: 研究动量空间电子波函数局域化产生的平带如何增强相互作用效应，以及自旋、电荷和晶格自由度强纠缠导致的竞争量子相

Method: 实验观测CrSb中M点(q*=(1/2 0))的短程电荷序涨落，测量TN处的声子色散Kohn异常，结合第一性原理计算分析电子-声子耦合机制

Result: 发现电荷序涨落在TN以上存在，进入磁有序相后消失；TN处q*点声子色散出现显著Kohn异常；TN以下交换伸缩效应使软声子模重整化约6 meV，为迄今报道的最大自旋-声子耦合

Conclusion: CrSb作为化学结构简单、对称性保护的交变磁体，成为研究平带如何介导巨大自旋-声子耦合和竞争破缺对称性的模型平台
...
### [30] [Stabilization of zigzag order in NiPS$_3$ via positive biquadratic interaction](https://arxiv.org/abs/2603.25475)
*Qiang Luo,Shuhang Yang,Xiaoying Wang,Zhengyu Jiang,Chunlan Ma,Yan Zhu*

Main category: cond-mat.str-el

TL;DR: 通过DFT计算和理论分析，确定了NiPS₃中双二次交换相互作用B/J₃≈0.44，该参数与J₁-J₃-B模型共同稳定了zigzag基态序。


<details>
  <summary>Details</summary>
Motivation: 尽管已有大量研究，但NiPS₃的自旋哈密顿量仍存在争议，特别是关于双二次交换项B的作用及其大小尚未明确。

Method: 采用密度泛函理论计算提取相互作用参数，结合最小增强自旋波理论和密度矩阵重整化群计算验证J₁-J₃-B模型。

Result: 获得正的双二次相互作用B/J₃≈0.44，该参数与铁磁最近邻J₁和反铁磁第三近邻J₃共同稳定了zigzag有序基态。

Conclusion: 虽然完全确定NiPS₃的自旋哈密顿量仍需进一步研究，但本文为理解其双二次相互作用提供了新见解，确认了B项在稳定zigzag序中的重要作用。
...
### [31] [Majorana-assisted nonlocal spin correlation in quasi-one-dimensional Kitaev spin liquids](https://arxiv.org/abs/2603.25647)
*Yuki Yamazaki,Shingo Kobayashi,Akira Furusaki*

Main category: cond-mat.str-el

TL;DR: 在准一维Kitaev自旋液体中，Majorana零模通过非局域自旋关联展现Majorana非局域性，该关联由Majorana费米子宇称算符映射到自旋弦算符产生。


<details>
  <summary>Details</summary>
Motivation: 探索Kitaev自旋液体中Majorana非局域性的具体物理表现，寻找可观测的实验特征来验证Majorana零模的存在及其非局域特性。

Method: 研究准一维蜂窝Kitaev模型的无磁通扇区，分析其拓扑性质，证明存在局域在两端的Majorana零模。将非局域Majorana费米子宇称算符映射到非局域自旋弦算符，在保持Majorana和总费米子宇称的微扰下，计算端到端自旋关联。

Result: 数值计算证实了由Majorana零模产生的有限非局域自旋关联，该关联在没有局域磁化的情况下存在，与Majorana费米子宇称算符和总费米子宇称算符的乘积成正比。

Conclusion: 该研究建立了量子自旋液体中内在Majorana非局域性的具体特征，为非局域Majorana零模的实验探测提供了理论依据。
...
### [32] [Pseudogap and Non-Fermi-liquid criticality in double Kondo model for bilayer nickelates](https://arxiv.org/abs/2603.25742)
*Jing-Yu Zhao,Ya-Hui Zhang*

Main category: cond-mat.str-el

TL;DR: 双层Kondo晶格模型的正常态相图研究，发现无层间隧穿时存在非费米液体临界点，区分过掺杂费米液体和欠掺杂赝隙金属（第二费米液体），并应用于双层镍酸盐La₃Ni₂O₇。


<details>
  <summary>Details</summary>
Motivation: 受双层镍酸盐高温超导实验进展的启发，研究双层Kondo晶格模型的正常态相图，探索非费米液体临界行为和赝隙金属相。

Method: 使用单点动力学平均场理论（DMFT）研究双层Kondo晶格模型，结合辅助费米子框架进行解析描述，分析层间自旋耦合J⊥和空穴掺杂x的相图。

Result: 发现无层间隧穿时存在非费米液体临界点，区分过掺杂标准费米液体和欠掺杂赝隙金属（第二费米液体）。赝隙相表现为重费米液体特征，具有小准粒子权重和大有效质量。提出La₃Ni₂O₇当前实验样品处于过掺杂费米液体区，赝隙相可通过电子掺杂实现。

Conclusion: 双层Kondo晶格模型展现出丰富的正常态相图，包括非费米液体临界点和赝隙金属相。这些结果为理解双层镍酸盐La₃Ni₂O₇的超导机制提供了理论框架，并预测赝隙相可通过电子掺杂实现。
...
### [33] [Chalcogen Doping Effect on the Insulator-to-Metal Transition in GdPS](https://arxiv.org/abs/2603.24762)
*Gokul Acharya,Rabindra Basnet,Santosh Karki Chhetri,Dinesh Upreti,M. M. Sharma,Jian Wang,David Graf,Jin Hu*

Main category: cond-mat.mtrl-sci

TL;DR: Se取代增强GdPS的自旋轨道耦合，略微增加磁各向异性，同时抑制场致绝缘体-金属转变，可能源于带隙增大需要更高交换分裂才能闭合。


<details>
  <summary>Details</summary>
Motivation: 研究GdPS中Se取代对自旋轨道耦合、磁各向异性及输运行为的影响，以深入理解这些物理参数之间的相互作用，为未来功能材料设计提供指导。

Method: 通过Se取代（预期增强自旋轨道耦合）研究GdPS的磁输运性质变化，包括磁各向异性测量和场致绝缘体-金属转变的抑制效应分析。

Result: Se取代略微增加了磁各向异性，同时抑制了场致绝缘体-金属转变，这可能是由于带隙增大需要更高的交换分裂才能闭合。

Conclusion: 研究揭示了自旋轨道耦合、磁各向异性和输运行为之间的复杂相互作用，为调控GdPS及相关材料的电子性质提供了重要见解，有助于未来设计具有特定功能的新型材料。
...
### [34] [Visualizing Millisecond Atomic Dynamics of Nanocrystals in Liquid](https://arxiv.org/abs/2603.24776)
...
TL;DR: 该研究通过深度学习去噪的毫秒级液体池电子显微镜，首次直接可视化了金纳米晶体在反应性液体环境中的原子尺度动态结构波动。


<details>
  <summary>Details</summary>
Motivation: 纳米材料的结构在化学物种和外部刺激作用下持续动态变化，随着尺寸和维度的减小，这种动态性进一步增强。尽管分析技术有所进步，但在反应性环境中同时实现原子空间分辨率和相应时间分辨率来捕捉纳米材料结构动态仍具挑战性。

Method: 采用毫秒级液体池电子显微镜结合深度学习去噪技术，直接可视化金纳米晶体在反应性液体环境中的原子尺度动态。

Result: 发现金纳米晶体的局部结晶度存在可逆波动，这种波动依赖于周围化学环境。这些由纳米晶体-液体界面相互作用驱动的瞬态波动，对溶解动力学和晶界弛豫具有关键影响。
...
TL;DR: 开发了适用于CrCoNi体系的通用机器学习原子间势（NEP），在宽成分范围内实现近第一性原理精度，能准确描述短程有序、层错能等成分依赖行为，为非等摩尔合金设计提供可靠模拟工具。


<details>
  <summary>Details</summary>
Motivation: CrCoNi中熵合金具有优异的力学性能，源于化学复杂性（如短程有序）和低层错能，但大规模原子模拟面临挑战。现有势函数多限于等摩尔成分，而偏离等摩尔比是调控性能的有效途径，需要能捕捉成分依赖行为的可迁移原子间势。

Method: 在神经进化势（NEP）框架下开发了CrCoNi体系的通用机器学习原子间势。基于自旋极化第一性原理数据，训练数据集涵盖纯元素、二元和三元合金的宽成分范围、多种晶体结构和热力学条件。

Result: 模型准确再现了状态方程、声子谱、弹性常数、位错解离、表面和缺陷能、熔化温度及应变诱导相变。能捕捉短程有序及其对层错能的影响，在等摩尔和非等摩尔成分下均与第一性原理和实验结果一致。相比现有势函数，在保持高效计算的同时，在整个成分空间提供一致精度。

Conclusion: 该模型实现了可靠的成分依赖行为原子模拟，为非等摩尔CrCoNi合金设计提供了框架，解决了现有势函数限于等摩尔成分且对纯元素精度不足的问题。
...
### [36] [Anisotropic light-electron-phonon coupling and ultrafast carrier separation in ferroelectric BaTiO$_3$](https://arxiv.org/abs/2603.25521)
*Atal Bihari Swain,Somnath Kale,Rohit Soni,Peter Baum*

Main category: cond-mat.mtrl-sci

TL;DR: BaTiO₃对光的响应具有偏振敏感性，电子-声子耦合和电子-空穴对运动受铁电极化方向影响


<details>
  <summary>Details</summary>
Motivation: 铁电材料的内建电场在超快电子学和太阳能电池中有重要应用，但光激发下电子-声子耦合和载流子动力学的偏振依赖性机制尚不清楚

Method: 使用超快电子衍射技术研究BaTiO₃的光响应，通过偏振敏感测量分析电子-声子耦合；利用超快电子静电测量可视化光激发电子-空穴对在铁电场中的运动

Result: 当光电场与铁电极化方向对齐时，激发电子更快地弛豫到声子和温度中；超快电子静电测量直接观测到光激发电子-空穴对在铁电场中的运动

Conclusion: 铁电BaTiO₃表现出偏振敏感的光响应特性，为利用铁电材料的内建电场调控超快电子过程和载流子动力学提供了新见解
...
### [37] [Suppression of Metallic Transport in Nitrogen-rich Two-Dimensional Transition Metal Nitrides](https://arxiv.org/abs/2603.24873)
...
TL;DR: 该研究揭示了二维过渡金属氮化物中氮含量调控的金属-半金属转变，以及低维下由无序诱导的输运机制和载流子类型转变。


<details>
  <summary>Details</summary>
Motivation: 二维过渡金属氮化物（如Mo5N6、δ-MoN、W5N6）的实验实现为探索其在二维极限下的基本物理性质提供了新机会。研究旨在建立这些材料输运现象的统一图像，理解氮含量对电子结构的影响。

Method: 结合实验观察和第一性原理计算：通过霍尔测量获得温度系数电阻（TCR）和磁阻（MR）分析低温输运机制；利用第一性原理计算研究氮含量对费米能级态密度的影响；计算体相和二维TMNs（带-NH端基）的载流子密度。

Result: 低温（10-30 K）输运由无序诱导机制主导；氮含量变化驱动金属到半金属的转变，氮富集TMNs（如Mo5N6）的费米能级态密度被抑制；Mo5N6在厚度减小时发生多数载流子类型转变，与霍尔测量结果一致。

Conclusion: 金属性钼氮化物中高氮含量可在二维极限下诱导向半金属相的转变，这为理解这些材料的基本物理性质和未来材料设计方向提供了重要见解。
...
### [38] [Concerted Electron-Ion Transport by Polyacrylonitrile Elucidated with Reactive Deep Learning Potentials](https://arxiv.org/abs/2603.24798)
*Rajni Chahal-Crockett,Michael D. Toomey,Logan T. Kearney,Yawei Gao,Joshua T. Damron,Amit K. Naskar,Santanu Roy*

Main category: cond-mat.mtrl-sci

TL;DR: 开发深度学习势能模型研究PAN环化动力学，发现亲核攻击是限速步骤，随后触发Li+耦合电子转移使后续环化加速~10,000倍


<details>
  <summary>Details</summary>
Motivation: 聚合物如PAN的电荷传输对电子和储能应用至关重要，但复杂反应性聚合物构型的基础作用尚不清楚，需要深入理解PAN环化动力学机制

Method: 开发基于从头算能量和力的深度学习势能模型，训练于非平衡反应性PAN构型，分析反应自由能、速率和电荷分布，并通过IR和NMR实验验证

Result: 亲核攻击产生第一个环是限速步骤，随后触发Li+耦合电子转移沿PAN骨架进行，使剩余腈基的连续环化加速约10,000倍；扩展构型中偶极和氢键相互作用最小化，实现快速动力学

Conclusion: 建立了设计具有增强电荷传输性能的反应性聚合物的途径，为能源应用中的聚合物设计提供理论基础
...
### [39] [Engineering Nonlinear Optical Responses via Inversion Symmetry Breaking in Bilayer Bi2Se3](https://arxiv.org/abs/2603.24834)
*Vineet Kumar Sharma,Alana Okullo,Barun Ghosh,Arun Bansil,Sugata Chowdhury*

Main category: cond-mat.mtrl-sci

TL;DR: 通过扭转、点缺陷插入或外加电场打破Bi₂Se₃双层结构的中心对称性，可解锁丰富的非线性光学响应，其峰值响应与GeS等基准2D材料相当，适用于太赫兹应用和下一代光伏器件。


<details>
  <summary>Details</summary>
Motivation: 天然非中心对称材料稀缺，需要开发工程化的二维系统用于非线性光学应用。Bi₂Se₃双层结构原本具有中心对称性，通过打破这种对称性可以解锁非线性光学响应。

Method: 采用三种方法打破Bi₂Se₃双层结构的中心对称性：1) 扭转至21.78°的莫尔角度；2) 插入点缺陷（如硒空位V_Se）；3) 施加外部电场。计算了这些工程化结构的位移电流和注入电流电导率。

Result: 扭转双层在21.78°角度下，位移电流和注入电流电导率分别为-14 nm·μA/V²和104×10⁸ nm·A/(V²·s)；硒空位缺陷的相应值为-190 nm·μA/V²和-170×10⁸ nm·A/(V²·s)。所有三种方法产生的峰值非线性光学响应均与GeS等基准2D材料相当，且具有宽带响应和螺旋度依赖的电流生成特性。

Conclusion: 通过工程化手段打破Bi₂Se₃双层结构的中心对称性，可以产生与现有基准材料相当的非线性光学响应，这些工程化双层结构是下一代二维光伏器件的可行候选材料。
...
### [40] [Efficient all-electron Bethe-Salpeter implementation using crystal symmetries](https://arxiv.org/abs/2603.24860)
*Jörn Stöhler,Stefan Blügel,Christoph Friedrich*

Main category: cond-mat.mtrl-sci

TL;DR: 该论文实现了全电子Bethe-Salpeter方程（BSE）在FLAPW方法中的计算，通过全电子混合基展开相互作用势，并利用晶体对称性加速双粒子哈密顿量的构建和对角化，显著提高了计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有FLAPW实现中的BSE计算通常采用简单的平面波基展开裸库仑势和屏蔽库仑势，这在一定程度上放弃了全电子描述。为了在FLAPW框架内实现真正的全电子BSE计算，需要开发新的方法。

Method: 1. 在全电子混合基中展开相互作用势；2. 利用群论工具将双粒子哈密顿量化为块对角形式；3. 通过晶体对称性加速哈密顿量构建和对角化；4. 支持自旋轨道耦合；5. 采用分布式内存并行计算。

Result: 1. 实现了全电子BSE在FLAPW中的计算；2. 对于Si，利用对称性将哈密顿量维度降低5倍，对角化速度提高125倍；3. 计算的Si和MoS₂激子结合能（22meV和76meV）比先前BSE研究更接近实验值；4. 验证了LiF、Si和MoS₂的光学吸收谱。

Conclusion: 该工作成功实现了全电子BSE在FLAPW方法中的高效计算，通过利用晶体对称性显著加速了计算过程，并获得了更准确的激子结合能，为精确计算光学性质提供了有力工具。
...
### [41] [The ground state of CuInP$_2$S$_6$ thin films: A study of the deep potential method](https://arxiv.org/abs/2603.24901)
*Shengxian Li,Jiaren Yuan,Tao Ouyang,Anlian Pan,Mingxing Chen*

Main category: cond-mat.mtrl-sci

TL;DR: 通过结合第一性原理计算和深度势能方法，研究发现多层CuInP2S6薄膜中，考虑声子自由能后，铁电态成为基态，解决了实验与DFT预测间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 二维铁电材料CuInP2S6在室温下具有面外铁电性，但实验观测到的净极化态与DFT预测的反铁电基态存在显著矛盾，需要解释这种差异并确定真实基态。

Method: 结合第一性原理计算和深度势能方法，计算CIPS薄膜的电子能和声子自由能，分析不同极化态（铁电、反铁电、铁电）的相对稳定性。

Result: 对于双层以上薄膜，反铁电态（内层反铁电有序、表面层铁电有序）具有最低电子能，但铁电态能量接近。考虑声子自由能后，铁电态在多层CIPS中成为能量有利的基态。

Conclusion: 振动熵的纳入使铁电态在有限温度下成为多层CIPS的稳定基态，解决了实验与DFT预测间的矛盾，这对理解CIPS铁电性质及其器件应用至关重要。
...
### [42] [Forster energy transfer boosts indirect anisotropic interlayer excitons in 2L-MoSe2/perovskite heterostructures](https://arxiv.org/abs/2603.24928)
*Yingying Chen,Zihao Jiao,Haizhen Wang,Dehui Li*

Main category: cond-mat.mtrl-sci

TL;DR: 通过ReS2介导的Förster共振能量转移，显著增强了2L-MoSe2/钙钛矿异质结构中层间激子的光致发光强度和光学各向异性。
...
Method: 利用ReS2作为能量转移介质，通过Förster共振能量转移(FRET)增强2L-MoSe2/钙钛矿异质结构中IXs的发射，并将ReS2的光学各向异性转移到目标系统。

Result: 室温下2L-MoSe2发射增强约8倍，78K时动量间接IXs发射强度几乎翻倍；成功将ReS2的光学各向异性转移到2L-MoSe2和IXs，获得约1.1的线性二向色性。

Conclusion: 该工作将IXs研究范围从强本征发射的直接带隙材料扩展到间接带隙系统，为实现高性能偏振敏感光电器件提供了新途径。
...
### [43] [Dual migration modes of unfaulted disconnections on curved twin boundaries](https://arxiv.org/abs/2603.24987)
*Hongrui He,Hao Lyu,Xueting Si*

Main category: cond-mat.mtrl-sci

TL;DR: 研究发现铝中相干孪晶界上未层错化位错（UFDs）的迁移行为取决于其核心结构：纯刃型位错通过热激活双扭折机制迁移，速度随温度单调增加；而含螺型偶极分量的位错能垒低8倍，通过连续核心结构转变实现随机双向运动，无系统温度依赖性。


<details>
  <summary>Details</summary>
Motivation: 晶界迁移控制着晶体材料的微观结构演化，直接影响强度和热稳定性等力学性能。位错作为晶界上响应局部曲率形成的线缺陷，已被认为是晶界迁移的关键载体。然而，不同核心结构的位错如何影响迁移机制和动力学尚不清楚。

Method: 采用分子动力学模拟结合推弹性能带（NEB）方法，研究铝中相干孪晶界上未层错化位错（UFDs）在高温下的滑移行为。通过NEB计算能量势垒，分析不同核心结构位错的迁移机制。

Result: 发现迁移行为存在显著分叉：纯刃型Burgers矢量的UFDs通过热激活双扭折机制迁移，迁移速度随温度单调增加；而含螺型偶极分量的UFDs能垒约低8倍，其核心结构在滑移过程中发生连续转变，产生随机双向运动，无系统温度依赖性。

Conclusion: 位错核心结构从根本上决定了孪晶界的迁移模式和动力学。这一发现为理解位错介导的晶界迁移提供了新的机理见解，表明核心结构是调控晶界迁移行为的关键因素。
...
### [44] [Effect of Pb doping on the crystallization process and thermoelectric properties of Ge2Sb2Te5 phase change material](https://arxiv.org/abs/2603.25123)
*M. Zhezhu,A. Vasil'ev,M. Yaprintsev,A. Musayelyan,E. Pilyuk,O. Ivanov*

Main category: cond-mat.mtrl-sci

TL;DR: 研究通过Pb掺杂调控Ge2Sb2Te5相变材料的结晶行为和热电性能，发现Pb掺杂能降低相变温度并提升热电功率因子


<details>
  <summary>Details</summary>
Motivation: Ge-Sb-Te合金相变材料在存储器件和热电应用中具有潜力，但需要调控其相变行为和热电性能以实现多功能应用

Method: 制备不同Pb掺杂浓度的GST薄膜，通过X射线衍射和拉曼光谱分析结晶行为，测量电阻率、塞贝克系数计算功率因子

Result: Pb掺杂降低了非晶-立方和立方-六方相变温度，促进了六方相在较低温度下形成；2.5 at.% Pb-GST在633K时获得最大功率因子1.3，并具有最高载流子迁移率

Conclusion: Pb掺杂能有效调控GST的结构相变和热电性能，展示了Pb-GST在结合相变存储和热电功能的应用潜力，如光热电器件和非易失性热电传感器
...
### [45] [Distinguishing apparent and hidden altermagnetism via uniaxial strain in $\mathrm{CsV_2Te_2O}$-family](https://arxiv.org/abs/2603.25136)
*San-Dong Guo,Yang Liu*

Main category: cond-mat.mtrl-sci

TL;DR: 提出利用面内单轴应变区分表观和隐藏交变磁性，发现表观交变磁性在应变下产生明显净磁矩，而隐藏交变磁性保持零净磁矩，为Cs₁₋δV₂Te₂O材料提供实验可行的区分策略。


<details>
  <summary>Details</summary>
Motivation: Cs₁₋δV₂Te₂O材料具有两种近简并基态磁构型（C型和G型），分别对应表观和隐藏交变磁性。需要实验可行的方法来区分这两种磁性状态，理解其物理特性差异。

Method: 提出利用面内单轴应变作为区分手段，通过第一性原理计算验证：在C型反铁磁CsV₂Te₂O中，单轴应变诱导的磁矩远大于先前研究的交变磁性半导体。同时研究类似晶体结构半导体在单轴应变下的电子态转变。

Result: 单轴应变下，表观交变磁性表现出明显的净磁矩（压磁效应），而隐藏交变磁性保持零净磁矩。C型反铁磁CsV₂Te₂O的应变诱导磁矩显著大于先前研究的交变磁性半导体。

Conclusion: 提供实验可行的策略区分Cs₁₋δV₂Te₂O中的表观和隐藏交变磁性，扩展了压磁效应的物理含义，可在实验可合成的KV₂Se₂O和Rb₁₋δV₂Te₂O中直接验证。
...
### [46] [Pulsed Laser Template Engineering- PLATEN](https://arxiv.org/abs/2603.25214)
...
TL;DR: PLATEN技术通过PLD在预图案化硅衬底上沉积功能氧化物薄膜，实现高深宽比图案化，避免了传统RIE对难蚀刻材料的限制。


<details>
  <summary>Details</summary>
Motivation: 功能无机材料（特别是氧化物）薄膜在光电子应用中至关重要，但许多材料难以通过传统反应离子刻蚀（RIE）实现高深宽比图案化，因为其组成元素不易形成挥发性化合物。

Method: 提出PLATEN技术：首先用RIE在硅衬底上预图案化高深宽比特征，然后利用脉冲激光沉积（PLD）的高度定向性，在图案化硅上沉积功能薄膜。PLD的强方向性使薄膜精确复制硅图案形貌而不覆盖侧壁。

Result: PLATEN可实现约50nm特征尺寸的图案复制，氧化物薄膜可复制80nm厚度的硅图案。超过80nm时，图案中部出现与膜厚相关而与特征尺寸无关的"腰部"收缩。在图案化硅衬底上实现了近乎单晶的氧化物生长。

Conclusion: PLATEN为难以蚀刻的材料提供了一种可行的图案化途径，展示了在活性光电子材料应用中的潜力，特别是在高深宽比功能氧化物薄膜图案化方面。
...
### [47] [Machine-Learned Interatomic Potentials for Predicting Physicochemical Properties of Molten Metal-Salt Systems for Calcium Electrolysis](https://arxiv.org/abs/2603.25330)
*M. Polovinkin,N. Rybin,D. Maksimov,F. Valiev,A. Khudorozhkova,M. Laptev,A. Rudenko,A. Shapeev*

Main category: cond-mat.mtrl-sci

TL;DR: 该研究使用基于机器学习矩张量势的分子动力学模拟，准确预测了钙生产关键熔体系统（Ca-Cu合金和CaCl₂-KCl电解质）的热力学和输运性质，为冶金应用中的液态系统计算探索提供了可靠框架。


<details>
  <summary>Details</summary>
Motivation: 高效电解装置设计需要准确的熔体性质数据，但高温实验成本高且耗时。需要开发计算替代方法以准确预测钙生产关键系统（Ca-Cu合金和CaCl₂-KCl电解质）的性质。

Method: 采用基于机器学习矩张量势（MTP）的分子动力学模拟，MTP在高精度密度泛函理论数据上训练。对Ca-Cu合金和CaCl₂-KCl电解质系统进行模拟，涵盖电解相关的温度和组成范围。

Result: MTP驱动的MD模拟准确再现了结构、热力学和输运性质，包括密度、径向分布函数、热容、热导率、离子电导率（电解质）、粘度和扩散系数。计算结果与实验数据的偏差在20%以内。

Conclusion: 计算与实验的强一致性验证了所提方法，为冶金应用中液态系统的计算探索和优化建立了稳健框架，提供了高效电解装置设计的可靠数据来源。
...
### [48] [Self-thermometry measurements of the adiabatic temperature change in first-order phase transition magnetocaloric materials](https://arxiv.org/abs/2603.25429)
...
TL;DR: 提出一种在商用VersaLab PPMS中通过测量时变磁化强度来确定磁热材料绝热温度变化ΔT_ad的新方法，适用于一级和二级相变材料，验证了Gd5Si2Ge2材料的测量准确性。


<details>
  <summary>Details</summary>
Motivation: 当前测量磁热效应的方法不便，需要不同仪器测量各个性质或定制设备，阻碍了磁制冷器件的发展。需要一种能在单一商用仪器中完整表征磁热材料的方法。

Method: 在Quantum Design的商用VersaLab PPMS中测量时变磁化强度，利用平衡磁化强度作为磁化-温度转换曲线，将先前技术扩展到一级相变材料Gd5Si2Ge2（具有显著磁滞效应）。

Result: 对Gd5Si2Ge2在0-1T磁场变化下测得峰值ΔT_ad为4.47K，与同成分样品的直接测量值误差在1%以内。方法可实现磁熵变、绝热温度变化和热容的完整表征。

Conclusion: 该方法能够在单一商用仪器中完整表征任何磁热材料（无论一级或二级相变），为磁制冷器件开发提供了便捷准确的测量手段。
...
### [49] [Single Atom Magnets on Thermally Stable Adsorption Sites: Dy on NaCl(100)](https://arxiv.org/abs/2603.25608)
...
Method: 使用NaCl(100)薄膜作为基底，研究不同吸附位点（Na替代位、Cl位、桥位）上单个Dy原子的磁性行为，测量其电子占据、易磁化轴和自旋弛豫时间。

Result: Na替代位上的Dy原子具有4f^9占据、面外易磁化轴、10秒的自旋弛豫时间；Cl位上的Dy原子具有4f^10占据、磁滞回线和550秒的自旋弛豫时间；两种位点均表现出慢磁弛豫。
...
### [50] [Martensitic-like transition between liquid crystalline and crystalline phases of prototypical discotic organic semiconductor](https://arxiv.org/abs/2603.25445)
...
TL;DR: 在HAT6盘状有机半导体中，发现液晶柱状六方相向晶体固体的转变可通过类马氏体机制实现，具有取向关联、结构可逆和超快动力学特征，速度比理论预测快七个数量级。


<details>
  <summary>Details</summary>
Motivation: 传统相变机制（成核生长或马氏体转变）仅限于晶体相之间。本文旨在探索液晶相向晶体相的转变是否也能通过类马氏体机制实现，这对于有机电子器件中大面积晶格排列具有重要意义。

Method: 将HAT6的柱状六方相在光刻定义的微通道中进行双轴取向，通过深度过冷诱导结晶。结合偏振光学显微镜和X射线散射研究转变机制。

Result: 在最大可达到的过冷度下，ColH-晶体相变速度达到~100微米/秒，比各向同性熔体生长理论预测快七个数量级。转变表现出马氏体转变特征：母相与子相之间的取向关联、结构可逆性和超快动力学。

Conclusion: 类马氏体转变可发生在液晶与晶体之间，比先前认为的更普遍。锚定液晶的类马氏体转变可用于在任意长距离上生长双轴取向的有机分子晶体，这对具有柱状六方相的高性能分子半导体器件具有重要意义。
...
### [51] [Berry curvature induced giant anomalous and spin texture driven Hall responses in the layered kagome antiferromagnet GdTi3Bi4](https://arxiv.org/abs/2603.25504)
*Shobha Singh,Shivam Rathod,Rong chen,Lipika,Sneh,Rie Y. Umetsu,Yan Sun,Kaustuv Manna*

Main category: cond-mat.mtrl-sci

TL;DR: GdTi3Bi4是一种层状Kagome反铁磁体，表现出场诱导一级相变、自旋团簇玻璃态、巨大反常霍尔电导(~8.6×10³ Ω⁻¹cm⁻¹)，以及贝里曲率和自旋织构驱动的霍尔响应。


<details>
  <summary>Details</summary>
Motivation: 探索层状Kagome磁体作为贝里曲率工程和非传统输运现象的平台，特别是寻找具有巨大反常霍尔效应和自旋织构物理的材料体系。

Method: 单晶生长、磁化强度测量、电输运表征、交流磁化率测量（频率/温度/磁场依赖）、霍尔分析、标度分析、第一性原理计算。

Result: 1. 观察到场诱导一级相变；2. 发现自旋团簇玻璃态磁性相，源于非共线自旋织构；3. 在2K下获得巨大反常霍尔电导σ_xy^A~8.6×10³ Ω⁻¹cm⁻¹；4. 标度分析显示斜散射和本征贝里曲率贡献共存；5. 第一性原理计算显示费米能级附近的平带，Gd离子的f电子贡献大的本征霍尔响应。

Conclusion: GdTi3Bi4是一种罕见的层状Kagome磁体，表现出贝里曲率诱导的巨大反常霍尔效应和自旋织构驱动的霍尔响应，为探索自旋织构物理和推进低维自旋电子学功能提供了多功能平台。
...
### [52] [Charge and spin photogalvanic effects in the p-wave magnet NiI2](https://arxiv.org/abs/2603.25516)
*Giuseppe Cuono,Srdjan Stavric,Javier Sivianes Castano,Julen Ibanez-Azpiroz,Paolo Barone,Andrea Droghetti,Silvia Picozzi*

Main category: cond-mat.mtrl-sci

TL;DR: NiI2的非共线自旋螺旋产生非中心对称性，导致反常铁电极化和eV尺度奇宇称自旋劈裂；非线性光学输运可直接探测这些效应，产生光电流和纯自旋流。


<details>
  <summary>Details</summary>
Motivation: 探索NiI2中由非共线自旋螺旋产生的独特电子性质（反常铁电极化、p波磁态、奇宇称自旋劈裂），并开发非线性光学方法直接探测和分离这些效应。

Method: 使用第一性原理计算，研究NiI2在光激发下的非线性光学输运特性，包括线性偏振光下的光电流和圆偏振光下的注入电流，以及纯自旋电流的产生。

Result: 自旋螺旋诱导的反演破缺产生线性偏振光下的光电流，电导率超过传统铁电体；圆偏振光下的注入电流源于自旋劈裂态间的螺旋选择性跃迁，直接暴露p波自旋纹理；预测了纯自旋电流，其流向随线性和圆偏振激发而反转。

Conclusion: 非线性光学输运可直接探测NiI2的自旋螺旋诱导效应，该材料能产生和调控纯自旋电流，是范德华异质结构中全光学自旋注入的理想平台。
...
### [53] [Atomic-Scale Insights into Copper Corrosion in Acidic Environment through Cryogenic Atom Probe Tomography of 3D-Electrodeposited Microcorrosion Cell](https://arxiv.org/abs/2603.25556)
...
TL;DR: 该研究开发了一种基于局部电沉积液体技术的微腐蚀电池，结合低温原子探针层析成像技术，实现了对铜在稀硫酸中腐蚀界面的三维纳米尺度表征。


<details>
  <summary>Details</summary>
Motivation: 腐蚀源于固液界面处的原子级反应，但由于无法在表征过程中保持瞬态界面状态，直接实验观察这些反应一直具有挑战性。需要先进的原子尺度分析技术来完善腐蚀模型。

Method: 采用局部电沉积液体技术制备微腐蚀电池，将皮升级电解质封装在密封金属微容器中，然后使用低温原子探针层析成像技术进行分析。以铜在充气稀硫酸中的腐蚀作为模型系统。

Result: 实现了腐蚀反应的三维纳米尺度映射，具有空间、化学和时间分辨率。观察到温度和时间依赖的界面演化，包括硫酸铜物种的纳米级聚集、高温下增强的离子配对，以及传统方法无法检测到的瞬态碳基界面复合物的出现。

Conclusion: 微腐蚀电池架构为研究广泛材料-液体系统中的受限电化学和降解过程提供了新策略，结合微加工和低温原子探针层析成像技术，能够捕获传统方法无法获得的瞬态界面状态。
...
### [54] [Growth and Kerr magnetometry of Mn2Au on a gold-capped Nb(001) substrate](https://arxiv.org/abs/2603.25618)
...
Method: 在Nb(001)衬底上外延生长Mn2Au，用Au层覆盖；通过中能电子衍射、俄歇电子谱和低能电子衍射表征生长质量和表面结构；蒸发Fe层形成交换耦合双层体系；通过场冷却设置交换偏置，用克尔显微镜观察磁畴结构。

Result: 观察到层状生长模式；Mn2Au/Fe双层体系表现出可设置的交换偏置效应；发现存在交换偏置和无交换偏置的区域，畴尺寸为数十微米；450K以上退火会减少Fe与Mn2Au的磁耦合区域；界面终止对交换耦合起关键作用。

Conclusion: Mn2Au的生长质量、界面结构和终止对Mn2Au/Fe双层的磁性能有重要影响，这些发现有助于优化自旋电子学器件的材料性能。
...
### [55] [Interfacial Polytype Engineering of Polymer-Derived SiC via Compositionally Complex MXene Templating](https://arxiv.org/abs/2603.25652)
*Yuxiang Gan,Jianyu Dai,Laxmi Sai Viswanadha,Congjie Wei,Kelvin Y. Xie,Jeremy Watts,Mohammad Naraghi,Chenglin Wu*

Main category: cond-mat.mtrl-sci

TL;DR: 通过引入TiVCrMoC3 MXene纳米片作为二维模板，在聚合物衍生碳化硅中实现了界面驱动的多型选择调控，显著提升了材料力学性能。


<details>
  <summary>Details</summary>
Motivation: 聚合物衍生碳化硅的多型选择控制具有挑战性，因为堆垛序列在成核前沿局部决定。需要开发新策略来调控SiC晶体结构演化。

Method: 在预陶瓷阶段引入成分复杂的TiVCrMoC3 MXene纳米片，通过放电等离子烧结（1900°C，70 MPa）处理。MXene部分转变为多组分碳化物结构，形成两种不同的异质界面状态：重构碳化物/SiC界面和共格MXene/SiC界面。

Result: MXene界面驱动了SiC多型演化：重构界面破坏堆垛序列并促进六方有序化，驱动α-SiC形成；共格界面保持立方堆垛。在最佳MXene负载量下，界面重构最显著，杨氏模量增加约82%，断裂韧性提高42%。

Conclusion: 通过二维碳化物模板进行界面多型工程是调控聚合物衍生陶瓷晶体结构演化的有前景途径，界面状态对多型选择和力学性能具有决定性影响。
...
### [56] [Electrostatic Photoluminescence Tuning in All-Solid-State Perovskite Transistors](https://arxiv.org/abs/2603.25718)
*Vladimir Bruevich,Dmitry Maslennikov,Beier Hu,Artem A. Bakulin,Vitaly Podzorov*

Main category: cond-mat.mtrl-sci

TL;DR: 基于外延单晶金属卤化物钙钛矿的全固态半导体器件，通过栅极电压可逆调控钙钛矿光致发光强度，实现65-98%的调制深度。


<details>
  <summary>Details</summary>
Motivation: 开发一种新型光电开关器件，利用电场调控光致发光而非电致发光，为金属卤化物钙钛矿在光子学和光电子学中开辟新的应用可能性。

Method: 采用外延单晶金属卤化物钙钛矿薄膜构建光致发光场效应晶体管，通过栅极电场静电调制界面移动电荷密度，从而调控光生载流子的辐射与非辐射复合通道。

Result: 栅极电压可高效改变非辐射界面复合速率，实现65-98%的光致发光强度调制（取决于温度），在优化栅压下可近乎完全消除非辐射损耗，获得大面积薄膜器件的高外量子效率。
...
TL;DR: 本文提出并解决了对称二元感知机的师生模型，将传统存储导向模型转变为具有保证解的有根推理问题，分析了无噪声贝叶斯最优机制和热噪声效应，绘制了相图并识别了二阶不稳定性和一阶相变的学习结构。


<details>
  <summary>Details</summary>
Motivation: 传统对称感知机模型主要关注存储容量，本文将其重新表述为师生模型，从而创建具有保证解的有根推理问题，可以系统研究学习动态和相变行为。

Method: 采用对称二元感知机的师生模型框架，包含两种区域标签，使用退火和淬火自由熵计算在高维极限下分析相图，控制参数包括样本密度α、对称超平面距离κ和温度T。

Result: 绘制了三维控制参数空间的相图，识别出学习过程由二阶不稳定性创建教师相关的次优状态，随后通过一阶相变实现完全对齐的鲁棒场景，揭示了次优解亚稳态与向有根构型熔化的相互作用。

Conclusion: 对称感知机师生模型提供了分析学习动态的新框架，揭示了相变结构如何依赖于势函数选择，这对基于蒙特卡洛的优化算法具有重要意义，特别是次优解的亚稳态特性。
